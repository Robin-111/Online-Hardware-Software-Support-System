...
List<Engineer> eng = new ArrayList<>();
		...
		try(Connection conn = DBUtil.provideConnection()) {
			...
			while(rs.next()) {
				...
				engg.setEngId(rs.getInt("engId"));
				engg.setName(rs.getString("name"));
				engg.setUserName(rs.getString("username"));
				engg.setPassword(rs.getString("password"));
				engg.setType(rs.getString("type"));
				engg.setLocation(rs.getString("location"));
...
String res = "Engineer not found.";
		...
		try(Connection conn = DBUtil.provideConnection()) {
			...
			if(x>0) {
				res = "Record of Engineer with id "+ engId +" deleted form Data.";
			}else {
...
List<Complaints> compl = new ArrayList<>();
...
String res = "Complaint Id can't be found. Please enter a valid complaint Id";
